document.addEventListener('DOMContentLoaded', function() {
  ...
  function processUserData(res, containerId) {
    ...
    if (shapes && Array.isArray(shapes)) {
        ...
        shapes.forEach(shapes => {
            ...
            <tr>
                <td>${shapes.name}</td>
                <td>${new Date(shapes.updated_at).toLocaleString()}</td>
                <td>
                    <button class="btn btn-warning" onclick="editshapes('${shapes._id}')"><i class="fa-solid fa-pen-to-square"></i></button>
                    <button class="btn btn-danger" onclick="deleteshapes('${shapes._id}')"><i class="fa-solid fa-trash"></i></button>
                </td>
            </tr>
            `;
        });
  ...
  function fetchUserData(page, query = '') {
      ...
      fetch(`http://localhost:3456/shapes/?page=${page}&per_page=${rowsPerPage}&field=name&sort_order=asc&q=${searchQuery}`, {
          method: 'GET',
          headers: {
...
window.deleteshapes = function(shapeId) {
    const token = localStorage.getItem('accessToken');
    if (!token) {
    ...
    if (confirm('Bạn có chắc chắn muốn xóa bộ dữ liệu này?')) {
        fetch(`http://localhost:3456/shapes/${shapeId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
if (btnSave) {
  btnSave.addEventListener('click', function() {
    ...
    fetch('http://localhost:3456/shapes', {
      method: 'POST',
      headers: {
...
function resetForm() {
    const inputField = document.getElementById('shapename');
    inputField.value = '';
...
window.editshapes = function(shapeId) {
    fetch(`http://localhost:3456/shapes/${shapeId}`)
        .then(response => response.json())
        .then(data => {
            const shapeName = data.data;

            // Cập nhật tiêu đề modal
            modalTitle.textContent = 'Chỉnh sửa họ';

            // Cập nhật giá trị của input
            document.getElementById('shapename').value = shapeName.name;
    ...
    const newName = document.getElementById('shapename').value;
