            ...
            rows += `
                <tr>
                    ...
                    <td>
                        ...
                        <button class="btn btn-danger" onclick="confirmDeleteUser('${user._id}')"><i class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>
            `;
        });
...
function fetchUserData(page, query = '') {
  ...
  fetch(`http://localhost:3456/users/?page=${page}&per_page=${rowsPerPage}&field=name&sort_order=asc&q=${searchQuery}`, {
      method: 'GET',
      headers: {
  ...
  window.confirmDeleteUser = function(userId) {
      ...
      if (confirm('Bạn có chắc chắn muốn xóa bộ dữ liệu này?')) {
          fetch(`http://localhost:3456/users/${userId}`, {
              method: 'DELETE',
              headers: {
                  'Authorization': `Bearer ${token}`
              }
...
document.getElementById('searchInput').addEventListener('input', function() {
  ...
  if (btnSave) {
    btnSave.addEventListener('click', function() {
      const nameUser = document.getElementById('nameUser').value.trim();
        ...
        const phonerUser = document.getElementById('phonenumber').value.trim();
        ...
        console.log('Phone:', phonerUser);

        // Check if all fields are filled
        if (!nameUser || !emailUser || !passwordUser || !phonerUser) {
        ...
        .then(data => {
            ...
            if (data.error === false) {
                ...
                fetchData(recentUrl);
                document.getElementById('flowerForm').reset(); // Reset lại form
            } else {
                if (data.detail && data.detail.email) {
                    alert(data.detail.email);
                } else {
  ...
  function resetForm() {
      const inputField = document.getElementById('nameUser');
      const inputFieldemail = document.getElementById('emailUser');
      inputField.value = '';
  ...
  window.editUser = function(userId) {
    ...
    .then(response => response.json())
    .then(data => {
        const nameUser = data.data;
        const emailUser = data.data;

        // Cập nhật tiêu đề modal
        modalTitle.textContent = 'Chỉnh tài khoản';

        // Cập nhật giá trị của input
        document.getElementById('nameUser').value = nameUser.name;
        document.getElementById('emailUser').value = emailUser.email;
    ...
    btnUpdate.addEventListener('click', function() {
        ...
        const newEmail = document.getElementById('emailUser').value;
