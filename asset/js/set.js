document.addEventListener('DOMContentLoaded', function() {
  ...
  function processUserData(res, containerId) {
      ...
      if (sets && Array.isArray(sets)) {
          ...
          sets.forEach(set => {
              ...
              <tr>
                  <td>${set.name}</td>
                  <td>${new Date(set.updated_at).toLocaleString()}</td>
                  <td>
                      <button class="btn btn-warning" onclick="editSet('${set._id}')"><i class="fa-solid fa-pen-to-square"></i></button>
                      <button class="btn btn-danger" onclick="deleteSet('${set._id}')"><i class="fa-solid fa-trash"></i></button>
                  </td>
              </tr>
              `;
          });
  ...
  function fetchUserData(page) {
      ...
      // Sử dụng currentPage và searchQuery ở đây để fetch dữ liệu
      fetchDataAndDisplay(`http://localhost:3456/sets/?q=${searchQuery}`, rowsPerPage, currentPage, 'tableRows', processUserData);
  }
  ...
  window.deleteSet = function(setId) {
      ...
      if (confirm('Bạn có chắc chắn muốn xóa bộ dữ liệu này?')) {
          fetch(`http://localhost:3456/sets/${setId}`, {
              method: 'DELETE',
              headers: {
...
document.getElementById('searchInput').addEventListener('input', function() {
  ...
  if (btnSave) {
    btnSave.addEventListener('click', function() {
      const setName = document.getElementById('nameset').value.trim();
      if (setName === '') {
      ...
      fetch('http://localhost:3456/sets', {
        method: 'POST',
        headers: {
  ...
  function resetForm() {
      const inputField = document.getElementById('nameset');
      inputField.value = '';
      modalTitle.textContent = 'Thêm bộ mới';
  ...
  window.editSet = function(setId) {
      fetch(`http://localhost:3456/sets/${setId}`)
          .then(response => response.json())
          .then(data => {
              const setName = data.data;

              // Cập nhật tiêu đề modal
              modalTitle.textContent = 'Chỉnh sửa bộ';

              // Cập nhật giá trị của input
              document.getElementById('nameset').value = setName.name;
      ...
      const newName = document.getElementById('nameset').value;
