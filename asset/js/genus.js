document.addEventListener('DOMContentLoaded', function() {
    ...
    if (genuses && Array.isArray(genuses)) {
        ...
        genuses.forEach(genus => {
            ...
            <tr>
                ...
                <td>
                    <button class="btn btn-warning" onclick="editGenus('${genus._id}')"><i class="fa-solid fa-pen-to-square"></i></button>
                    <button class="btn btn-danger" onclick="confirmDeleteGenus('${genus._id}')"><i class="fa-solid fa-trash"></i></button>
                </td>
            </tr>
            `;
        });
  ...
  function fetchUserData(page, query = '') {
      ...
      fetch(`http://localhost:3456/genuses/?page=${page}&per_page=${rowsPerPage}&field=name&sort_order=asc&q=${searchQuery}`, {
          method: 'GET',
          headers: {
...
window.confirmDeleteGenus = function(genusId) {
    const token = localStorage.getItem('accessToken');
    if (!token) {
...
if (btnSave) {
  btnSave.addEventListener('click', function() {
    const genusName = document.getElementById('namegenus').value.trim();
    if (genusName === '') {
    ...
    fetch('http://localhost:3456/genuses', {
      method: 'POST',
      headers: {
...
function resetForm() {
    const inputField = document.getElementById('namegenus');
    inputField.value = '';
    modalTitle.textContent = 'Thêm họ mới';
    btnSave.style.display = 'inline-block';
    btnUpdate.style.display = 'none';
}

// Xử lý sự kiện khi người dùng nhấp vào nút chỉnh sửa
window.editGenus = function(genusId) {
    fetch(`http://localhost:3456/genuses/${genusId}`)
        .then(response => response.json())
        .then(data => {
            const genusName = data.data;

            // Cập nhật tiêu đề modal
            modalTitle.textContent = 'Chỉnh sửa họ';

            // Cập nhật giá trị của input
            document.getElementById('namegenus').value = genusName.name;
    ...
    const newName = document.getElementById('namegenus').value;
