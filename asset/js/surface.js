document.addEventListener('DOMContentLoaded', function() {
  ...
  function processUserData(res, containerId) {
    ...
    if (surfaces && Array.isArray(surfaces)) {
        let rows = '';
        surfaces.forEach(surfaces => {
            rows += `
            <tr>
                <td>${surfaces.name}</td>
                <td>${new Date(surfaces.updated_at).toLocaleString()}</td>
                <td>
                    <button class="btn btn-warning" onclick="editsurfaces('${surfaces._id}')"><i class="fa-solid fa-pen-to-square"></i></button>
                    <button class="btn btn-danger" onclick="deletesurfaces('${surfaces._id}')"><i class="fa-solid fa-trash"></i></button>
                </td>
            </tr>
            `;
        });
  ...
  function fetchUserData(page, query = '') {
      ...
      fetch(`http://localhost:3456/surfaces/?page=${page}&per_page=${rowsPerPage}&field=name&sort_order=asc&q=${searchQuery}`, {
          method: 'GET',
          headers: {
...
window.deletesurfaces = function(surfaceId) {
    const token = localStorage.getItem('accessToken');
    if (!token) {
    ...
    if (confirm('Bạn có chắc chắn muốn xóa bộ dữ liệu này?')) {
        fetch(`http://localhost:3456/surfaces/${surfaceId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
if (btnSave) {
  btnSave.addEventListener('click', function() {
    const surfaceName = document.getElementById('surfacename').value.trim();
    if (surfaceName === '') {
    ...
    fetch('http://localhost:3456/surfaces', {
      method: 'POST',
      headers: {
    ...
    .then(data => {
      ...
      if (data.error === false) {
        alert('Thêm hình dáng mới thành công!');
...
function resetForm() {
    const inputField = document.getElementById('surfacename');
    inputField.value = '';
...
window.editsurfaces = function(surfaceId) {
    fetch(`http://localhost:3456/surfaces/${surfaceId}`)
        .then(response => response.json())
        .then(data => {
            const surfaceName = data.data;

            // Cập nhật tiêu đề modal
            modalTitle.textContent = 'Chỉnh sửa bề mặt';

            // Cập nhật giá trị của input
            document.getElementById('surfacename').value = surfaceName.name;
    ...
    const newName = document.getElementById('surfacename').value;
