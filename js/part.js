document.addEventListener('DOMContentLoaded', function() {
  ...
  function processUserData(res, containerId) {
    ...
    if (parts && Array.isArray(parts)) {
        ...
        parts.forEach(parts => {
            ...
            <tr>
                <td>${parts.name}</td>
                <td>${new Date(parts.updated_at).toLocaleString()}</td>
                <td>
                    <button class="btn btn-warning" onclick="editparts('${parts._id}')"><i class="fa-solid fa-pen-to-square"></i></button>
                    <button class="btn btn-danger" onclick="deleteparts('${parts._id}')"><i class="fa-solid fa-trash"></i></button>
                </td>
            </tr>
            `;
        });
  ...
  function fetchUserData(page, query = '') {
      ...
      fetch(`http://localhost:3456/parts/?page=${page}&per_page=${rowsPerPage}&field=name&sort_order=asc&q=${searchQuery}`, {
          method: 'GET',
          headers: {
...
window.deleteparts = function(partId) {
    const token = localStorage.getItem('accessToken');
    if (!token) {
    ...
    if (confirm('Bạn có chắc chắn muốn xóa bộ dữ liệu này?')) {
        fetch(`http://localhost:3456/parts/${partId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
if (btnSave) {
  btnSave.addEventListener('click', function() {
    ...
    fetch('http://localhost:3456/parts', {
      method: 'POST',
      headers: {
...
function resetForm() {
    const inputField = document.getElementById('partname');
    inputField.value = '';
    modalTitle.textContent = 'Thêm phần mới';
    btnSave.style.display = 'inline-block';
    btnUpdate.style.display = 'none';
}

// Xử lý sự kiện khi người dùng nhấp vào nút chỉnh sửa
window.editparts = function(partId) {
    fetch(`http://localhost:3456/parts/${partId}`)
        .then(response => response.json())
        .then(data => {
            const partName = data.data;

            // Cập nhật tiêu đề modal
            modalTitle.textContent = 'Chỉnh sửa phần';

            // Cập nhật giá trị của input
            document.getElementById('partname').value = partName.name;
