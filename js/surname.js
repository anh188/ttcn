document.addEventListener('DOMContentLoaded', function() {
  ...
  function processUserData(res, containerId) {
    const surnames = res.data.surnames;
    if (surnames && Array.isArray(surnames)) {
        let rows = '';
        surnames.forEach(surnames => {
            rows += `
            <tr>
                <td>${surnames.name}</td>
                <td>${new Date(surnames.updated_at).toLocaleString()}</td>
                <td>
                    <button class="btn btn-warning" onclick="editsurnames('${surnames._id}')"><i class="fa-solid fa-pen-to-square"></i></button>
                    <button class="btn btn-danger" onclick="deletesurnames('${surnames._id}')"><i class="fa-solid fa-trash"></i></button>
                </td>
            </tr>
            `;
        });
  ...
  function fetchUserData(page, query = '') {
      ...
      fetch(`http://localhost:3456/surnames/?page=${page}&per_page=${rowsPerPage}&field=name&sort_order=asc&q=${searchQuery}`, {
          method: 'GET',
          headers: {
...
window.deletesurnames = function(surnameId) {
    const token = localStorage.getItem('accessToken');
    if (!token) {
...
if (btnSave) {
  btnSave.addEventListener('click', function() {
    const surnameName = document.getElementById('namesurname').value.trim();
    if (surnameName === '') {
    ...
    fetch('http://localhost:3456/surnames', {
      method: 'POST',
      headers: {
...
function resetForm() {
    const inputField = document.getElementById('namesurname');
    inputField.value = '';
